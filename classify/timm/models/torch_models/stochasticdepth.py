...
class StochasticDepthBasicBlock(nn.Module):#torch.jit.ScriptModule
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            x = self.residual(x) + self.shortcut(x)
            #x = torch.mul(self.residual(x), self.p) #+ self.shortcut(x)
            # x2 = 
            # x = x1 + x2
...
class StochasticDepthBottleNeck(nn.Module):#torch.jit.ScriptModule
...
class StochasticDepthResNet(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=100):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1)) #F.adaptive_avg_pool2d() #
    ...
    def forward(self, x):
        ...
        output = self.avg_pool(output) #F.adaptive_avg_pool2d(output,(1,1)) #
...
def Model(args):
    ...
    num_layers = str(args['model_cfg'].get('layer_num',18))
    return eval('stochastic_depth_resnet'+num_layers)(num_classes)