...
class Model(nn.Module):
    def __init__(self, cfg, ch=3, nc=None):  # model, input channels, number of classes
        ...
        head_num        = cfg.multi_head['num']
        loss_param      = eval('cfg.'+ cfg.version_info[version])

        try:
            exec(f'from yolodet.models.multi_head_utils.torch_{model_name} import YOLO')
        except:
            exec(f'from yolodet.models.multi_head_utils.{model_name} import YOLO')

        self.names = [str(i) for i in range(nc)]  # default names
        self.model = eval('YOLO')(nc, anchors, ch, heads=cfg.multi_head)
    
        # detect_layer_list = list(self.model.named_modules())[-1*head_num:] #detect layer
        # # Build strides, anchors
        # for detect_layer in detect_layer_list:#对每一组的检测头进行初始化
        #     m = eval('self.model.' + detect_layer[0]) # Detect()
        #     m.version = version
        ...
        #         if not loss_param.get('use_sigmoid', True): #Gfocal_v2配置
        #             m.box_coder = GFLHead(num_classes=nc, anchor_generator=np.array(anchors).reshape(len(anchors),-1,2).tolist(), **loss_param)
        #     else:
