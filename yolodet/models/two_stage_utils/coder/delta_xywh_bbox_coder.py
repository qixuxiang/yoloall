...
def bbox2delta(proposals, gt, means=(0., 0., 0., 0.), stds=(1., 1., 1., 1.)):
    ...
    dx = (gx - px) / pw #x方向（是在anchor的w）的归一化
    dy = (gy - py) / ph #y方向(是在anchor的h)的归一化
    dw = torch.log(gw / pw)
    dh = torch.log(gh / ph)
    ...
    deltas = deltas.sub_(means).div_(stds) #减均值除方差

    return deltas


# @mmcv.jit(coderize=True)
def delta2bbox(rois, # anchor
               deltas, # propersal的框(经过encode的)
               means=(0., 0., 0., 0.),
               stds=(1., 1., 1., 1.),
               max_shape=None,
               wh_ratio_clip=16 / 1000,
    ...
    gxy = pxy + dxy_wh
    gwh = pwh * dwh.exp()
    x1y1 = gxy - (gwh * 0.5) # 将中心点在转化为左上点和右下点
    x2y2 = gxy + (gwh * 0.5)
