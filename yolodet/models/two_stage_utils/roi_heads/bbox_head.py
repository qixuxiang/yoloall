...
from yolodet.models.two_stage_utils.coder import DeltaXYWHBBoxCoder
from yolodet.loss.mmdet_loss_utils import CrossEntropyLoss, L1Loss, accuracy
...
class BBoxHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 loss_cls=dict(
                     ...
                     reduction='mean'),
                 loss_bbox=dict(
                     type='SmoothL1Loss', beta=1.0, loss_weight=1.0, reduction='mean'),
                 init_cfg=None):
        ...
        self.bbox_coder = DeltaXYWHBBoxCoder(target_means=bbox_coder['target_means'],
                                             target_stds=bbox_coder['target_stds'])

        self.loss_cls = CrossEntropyLoss(use_sigmoid=loss_cls['use_sigmoid'],
                                         loss_weight=loss_cls['loss_weight'],
                                         reduction=loss_cls['reduction'])

        self.loss_bbox = L1Loss(loss_weight=loss_bbox['loss_weight'], reduction=loss_cls['reduction'])
        ...
        if self.with_cls:
            ...
            self.fc_cls = nn.Linear(in_features=in_channels, out_features=cls_channels, bias=True)
            # build_linear_layer(
        ...
        if self.with_reg:
            ...
            self.fc_reg = nn.Linear(in_features=in_channels, out_features=out_dim_reg, bias=True)
            # build_linear_layer(
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
        ...
        # 根据gt的标签和box  对 roi筛选出的框进行匹配：结果为labels值，label_weights, bbox_targets（与gt_box的encode【中心点和w,h的差值】），boxes_weights
        ...
        if num_pos > 0:
            ...
            pos_weight = 1.0 if cfg['pos_weight'] <= 0 else cfg['pos_weight'] #cfg.pos_weight
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                losses['loss_bbox'] = self.loss_bbox(
                    ...
                    # avg_factor=bbox_targets.size(0), #走sum时不需要avg_factor，走mean时需要avg_factor
                    reduction_override=reduction_override)
            else:
