...
class AnchorGenerator:
    ...
    def grid_priors(self, featmap_sizes, dtype=torch.float32, device='cuda', level=None):
        ...
        """
        if level != None:
            return self.single_level_grid_priors(featmap_sizes, level, dtype=dtype, device=device)
        else:
...
class YOLOAnchorGenerator(AnchorGenerator):
    ...
    def single_level_responsible_flags(self,
        ...
        responsible_grid = responsible_grid[None,:].expand(
            num_base_anchors, responsible_grid.size(0)).contiguous().view(-1)
