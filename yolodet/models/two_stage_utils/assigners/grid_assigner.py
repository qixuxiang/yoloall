...
class GridAssigner(BaseAssigner):
    ...
    def assign(self, bboxes, box_responsible_flags, gt_bboxes, gt_labels=None):
        ...
        max_overlaps, argmax_overlaps = overlaps.max(dim=0) # value , index 列

        if isinstance(self.neg_iou_thr, float):
            assigned_gt_inds[(max_overlaps >= 0)
                             & (max_overlaps <= self.neg_iou_thr)] = 0 #处于[0,0.5]为负样本
        ...
        # unrelated anchors, i.e. not box_responsible_flags # 每个位置目标的响应
        overlaps[:, ~box_responsible_flags.type(torch.bool)] = -1. #位置取反操作 -> 有目标的位置保留，不关心的置-1

        # calculate max_overlaps again, but this time we only consider IOUs
        # for anchors responsible for prediction
        max_overlaps, argmax_overlaps = overlaps.max(dim=0) #返回每列的最大value 和 index

        # for each gt, which anchor best overlaps with it
        # for each gt, the max iou of all proposals
        # shape of gt_max_overlaps == gt_argmax_overlaps == num_gts
        gt_max_overlaps, gt_argmax_overlaps = overlaps.max(dim=1) #返回每行的最大value 和 index

        pos_inds = (max_overlaps >
                    self.pos_iou_thr) & box_responsible_flags.type(torch.bool) # 保留正样本并且存在响应的目标index
        assigned_gt_inds[pos_inds] = argmax_overlaps[pos_inds] + 1 #加1是由于索引从0开始的
