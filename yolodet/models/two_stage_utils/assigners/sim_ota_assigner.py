...
class SimOTAAssigner(BaseAssigner):
    ...
    def _assign(self,
        ...
        with autocast(enabled=False):   # 加入autocast为防止报不同数据类型溢出的错误：torch.nn.functional.binary_cross_entropy和torch.nn.BCELoss不能进行安全的自动转换（16位浮点与32浮点之间的互转），让你使用torch.nn.functional.binary_cross_entropy_with_logits或torch.nn.BCEWithLogitsLoss 
            cls_cost = F.binary_cross_entropy( # Lcls  binary_cross_entropy #binary_cross_entropy_with_logits
    ...
    def get_in_gt_and_in_center_info(self, priors, gt_bboxes):#gt_bboxes ->为（左上点和右下点）
        num_gt = gt_bboxes.size(0)
