...
class CSPLayer_tsh(nn.Module):
    ...
    def __init__(
        ...
        # self.conv3 = BaseConv(2 * hidden_channels, out_channels, 1, stride=1, act=act)
        if FPN:
            self.conv3 = BaseConv(hidden_channels, out_channels * 2, 1, stride=1, act=act)
        else:
...
class CSPLayer_FPN0(nn.Module):
    ...
    def __init__(
        ...
        module_list = [
            Bottleneck(
                out_channels, out_channels, shortcut, 1.0, depthwise, act=act
            )
            for _ in range(n)
        ]
        self.m = nn.Sequential(*module_list)
...
class YOLO(nn.Module):
    def __init__(self, nc, anchors, ch):
        ...
        self.FPN0 = CSPLayer_FPN0(
            # int(2 * in_channels[1] * width),
            int(in_channels[2] * 1*width),
            int(in_channels[1] * 1*width),
            round(1),#3 * depth
            False,
            expansion=1,
            depthwise=depthwise,
        ...
        self.reduce_conv2 = BaseConv(
            int(in_channels[0] * 0.5), int(in_channels[0]), 1, 1, act=act
        )
        self.reduce_conv3 = BaseConv(
            int(in_channels[0]), int(in_channels[1]), 1, 1, act=act
        )
        ...
        self.detect = Detect(nc, anchors, [int(in_channels[0]), int(in_channels[1]), int(in_channels[2])])
    ...
    def forward(self, input):
        ...
        f_out1 = self.upsample(fpn_out1)  # 256/8
        # f_out1 = torch.cat([f_out1, x2], 1)  # 256->512/8
        f_out1 = f_out1 + self.reduce_conv2(x2)
        ...
        p_out0 = self.bu_conv1(pan_out1)  # 512->512/32
        # p_out0 = torch.cat([p_out0, fpn_out0], 1)  # 512->1024/32
        p_out0 = p_out0 + self.reduce_conv3(fpn_out0)
