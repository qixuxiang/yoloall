...
def create_optimizer(opt, model, epochs, ckpt, results_file):
    ...
    if not opt.distiller['teacher_init']:#互学习开启
        model = model.student
    for k, v in model.named_modules():#学生网络的优化FPN层学习率尽量的小，主要起到微调的效果
        ...
        if isinstance(v, nn.BatchNorm2d) or isinstance(v, nn.LayerNorm):# 这里nn.LayerNorm
            if k in distill_layer:
                ...
                pg0.append(v.weight)  # no decay BN层优化
