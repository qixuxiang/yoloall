...
def test(opt,
    ...
    names = {k: v for k, v in enumerate(model.backbone.names if hasattr(model.backbone, 'names') else model.module.backbone.names)}
    ...
    for batch_i, (img, targets, paths, metas) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            t = time_synchronized()
            if two_stage_enabel:
                if epoch > two_stage_start + 1000:
                    proposal, train_out, feature_list = model.backbone(img) #生成的框为xywh

                    #inf_out = torch.cat(proposal,1)# 将各个feature_map上预测的框进行cat
                    ...
                    #proposal_list = torch.cat([proposal_list[level][img_num] for 
                    # # 将多有框进行iou剔除，返回的为框的index和iou的分数
                    # proposal_box = [xywh2xyxy(proposal[...,:5][i]) for i in range(len(proposal))] # (xywh) to (xyxy)
                    # proposal_list =[proposal_box[img_idx][proposal_box[img_idx][:,4] > 0.1] for img_idx in range(len(proposal_box))]

                    det_bboxes = model(feature_map=feature_list,proposal_list=proposal_list,img_metas=img_metas,train=False)

                    if 1:
                        import cv2
                        for image_num in range(len(img_metas)):
                            img_shape = img_metas[image_num]['img_shape'] #(h,w)
                            pad_shape = img_metas[image_num]['pad_shape']
                            t_pad = (pad_shape[0] - img_shape[0]) // 2 #h
                            l_pad = (pad_shape[1] - img_shape[1]) // 2 #w
                            image = cv2.imread(img_metas[image_num]['filename'])
                            #image = cv2.resize(image,(img_shape[1],img_shape[0]))

                            for class_type in range(len(det_bboxes[image_num])):
                                if det_bboxes[image_num][class_type].shape[0] > 0:
                                    for box in det_bboxes[image_num][class_type]:
                                        image = cv2.rectangle(image,(int(box[0]-l_pad),int(box[1]-t_pad)),(int(box[2]-l_pad),int(box[3]-t_pad)),(0,255,0),1)
                                        #img = cv2.rectangle(img,(int(pos_bbox[0]-l_pad),int(pos_bbox[1]-t_pad)),(int(pos_bbox[2]-l_pad),int(pos_bbox[3]-t_pad)),(0,0,255),1)
                                        image = cv2.putText(image, str(box[4]), (int(box[0]-l_pad),int(box[1]-t_pad)), 0, 1, (0,0,255), 1)
                    ...
                    inf_out, train_out, feature_list = model(img=img,img_metas=img_metas,train=False)  # inference and training outputs
                    inf_out = torch.cat(inf_out, 1)
            else:
                inf_out, train_out = model.backbone(img, augment=augment)  # inference and training outputs 这里augment参数需要查明
