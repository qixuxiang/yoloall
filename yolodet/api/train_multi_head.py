...
from yolodet.loss.loss_multi_base import ComputeLoss
...
def train_multi_head(opt, device, tb_writer=None):
    # Show parmter
    show_opts(opt)
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, model.stride, opt, augment=True, rank=rank)
        ...
        for i, (imgs, targets, paths, metas) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                if 'mmdet' in loss_version:
                    ...
                    img_metas = metas
                    gt_labels_multi, gt_bboxes_multi = targets

                    loss = imgs.new_zeros(1)
                    logstrs = []
                    loss_items = []
                    box_num = []

                    for hi in range(opt.multi_head['num']):
                        gt_labels = []
                        gt_bboxes = []

                        for idx in range(len(img_metas)):
                            gt_labels.append(gt_labels_multi[idx][hi].to(device).long())
                            gt_bboxes.append(gt_bboxes_multi[idx][hi].to(device))
                        
                        box_num.append(sum([_.shape[0] for _ in gt_bboxes]))

                        loss_multi, loss_items_multi = compute_loss(pred[hi], [gt_bboxes,gt_labels], img_metas, head_index=hi)
                        head_weight = opt.multi_head['head_weights'][hi]
                        loss_multi *= head_weight / opt.multi_head['num'] # head mean
                        loss_items_multi *= head_weight / opt.multi_head['num'] # head mean

                        loss += loss_multi
                        loss_items.append(loss_items_multi)
                        # logstrs.append(f'head {hi}, weight {head_weight}:')
                        # logstrs.extend(logstrs_multi)
            ...
            if rank in [-1, 0] and item_num % 5 == 0:
                ...
                for num_space in range(len(train_info)-1):
                    space += ' '
                for loss_i in range(len(loss_items)):
                    mloss = (mloss * i + loss_items[loss_i]) / (i + 1)  # update mean losses
                    #mem = '%.3g' % (torch.cuda.memory_reserved() / 1E9 if torch.cuda.is_available() else 0)  # (GB)
                    if 'mmdet' in loss_version:
                        targets_num = box_num[loss_i]
                    s = ('%10s' * 3 + '%10.4g' * 7) % (
                        '%g/%g' % (epoch, epochs - 1), mem, '%g/%g' % (item_num,len(dataloader)), *mloss, targets_num, optimizer.param_groups[0]['lr'], imgs.shape[-1])
                    s_list.append(s)
                    head_info = ('box: %-9.6g' + ' obj:%-9.6g' + ' cls:%-9.6g' + ' total:%-9.6g' + 'tgs:%-5g' + 'lr:%-9.6g' + '  size:%-3g %-3g' + '  t: %-4.3gs') % (*mloss, targets_num, optimizer.param_groups[0]['lr'], imgs.shape[-1], imgs.shape[-2], time.time()-t1)
                    if loss_i == 0:
                        train_infos = train_info + head_info
                        logger.info(train_infos)
                    else:
                        logger.info(space + head_info)
                ...
                if plots and ni < 3:
                    if 'mmdet' in loss_version:
                        targets_total = []
                        img_num = 0
                        for labels_gts,boxes_gts in zip(targets[0],targets[1]):
                            labels_gt = torch.cat(labels_gts,0)
                            boxes_gt = torch.cat(boxes_gts,0)
