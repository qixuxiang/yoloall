...
def create_optimizer(opt, model, epochs, ckpt, results_file):
    ...
    if weights_one and (weights_one.endswith('.pth') or weights_one.endswith('.pt')):
        ...
        del ckpt
    # 如果backbone和检测头的参数不一样可以放进来定义检测头的参数
    if weights_two:
        pass
...
def train_two_stage(opt, device, tb_writer=None):
    ...
    # Create optimizer （当前二阶段只对backbone设置优化参数）
    ...
    # DDP mode
    if cuda and rank != -1:
        model = DDP(model, device_ids=[opt.local_rank], output_device=opt.local_rank, find_unused_parameters=True) #添加find_unused_parameters=true
    ...
    if (cuda and rank != -1) or (cuda and rank == -1 and torch.cuda.device_count() > 1):
        ...
        model.module.backbone.class_weights = labels_to_class_weights(dataset.labels, nc).to(device) * nc  # attach class weights 待考虑
        ...
        model.backbone.class_weights = labels_to_class_weights(dataset.labels, nc).to(device) * nc  # attach class weights 待考虑
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                if 'mmdet' in loss_version:
                    ...
                    for idx in range(len(img_metas)):
                        gt_labels[idx] = gt_labels[idx].type(torch.int64).to(device) #label转化为torch.int64
        ...
        if rank in [-1, 0]:
            ...
            with open(results_file_per_class, 'a') as f:
                ...
                #f.write('\n')
                    # Plots
            if plots and (epoch+1) % 5 == 0: #每5轮保存一下训练各个类型的结果
