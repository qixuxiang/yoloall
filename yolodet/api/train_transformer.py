...
from yolodet.optimizer.optimizer_transformer import create_swin_optimizer
import pdb
...
def create_model(opt,device):
    ...
    if model_name.lower() == 'swin':
        from yolodet.models.swin import Model
        logger.info(f'Loading model from yolodet.models.{model_name} import Model')
    ...
    if pretrained:
        ...
        ckpt = None
        model = Model(nc=nc,anchors=opt.train_cfg['anchors'],version=opt.train_cfg['version'],**opt.transformer['transformer_cfg'])
        model.init_weights()
        model.to(device)
...
def create_optimizer(opt, model, epochs, ckpt, results_file, init_optimizer):

    pg0, pg1, pg2 = [], [], []  # optimizer parameter groups
    optim_model_modules = [model.detect] # 选择特定的优化的模型结构块如：neck、roi_head、detect_head
    for model_patch in optim_model_modules:
        for k, v in model_patch.named_modules():
    ...
    # Tramsformer 选用adamw作为backbone网络的优化器
    optimizer = optim.AdamW(**init_optimizer)
    # if len(pg0) > 0:
    #     optimizer.add_param_group({'params': pg0, 'lr': opt.hyp['lr0'], 'betas':(opt.hyp['momentum'], 0.999)})

    # if opt.hyp['optim'] == 'adam':
    #     optimizer.add_param_group({'params': pg0, 'lr': opt.hyp['lr0'], 'betas':(opt.hyp['momentum'], 0.999)})
    #     #optimizer = optim.Adam(pg0, lr=opt.hyp['lr0'], betas=(opt.hyp['momentum'], 0.999))  # adjust beta1 to momentum
    # elif opt.hyp['optim'] == 'adamw':
    #     optimizer.add_param_group({'params': pg0, 'lr': opt.hyp['lr0'], 'betas':(opt.hyp['momentum'], 0.999)})
    # else:
    #     optimizer.add_param_group({'params': pg0, 'lr': opt.hyp['lr0'], 'betas':(opt.hyp['momentum'], 0.999)})
    #     #optimizer = optim.SGD(pg0, lr=opt.hyp['lr0'], momentum=opt.hyp['momentum'], nesterov=True)
    # if 'L2' in opt.hyp['norm'] and len(pg1) > 0:
...
def train_transformer(opt, device, tb_writer=None):
    # Show parmter
    show_opts(opt)
    ...
    # Image sizes
    gs = max(model.strides_list)  # grid size (max stride)
    ...
    #Creat transformer optimizer (这里设置初始为整个网络的optimizer的参数)
    optimizer_cfg = opt.transformer['optimizer_cfg'] #{'lr':0.0001, 'betas':(0.9, 0.999), 'weight_decay':0.05}
    paramwise_cfg = opt.transformer['paramwise_cfg'] #{'absolute_pos_embed': {'decay_mult': 0.0}, 'norm': {'decay_mult': 0.0}, 'relative_position_bias_table': {'decay_mult': 0.0}}
    transformer_optimizer = create_swin_optimizer(optimizer_cfg, paramwise_cfg)
    init_optimizer = transformer_optimizer(model)
    # Create optimizer
    start_epoch, best_fitness, optimizer, lf, scheduler = create_optimizer(opt, model, epochs, ckpt, results_file, init_optimizer)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if opt.train_cfg['test'] or final_epoch:  # Calculate mAP
                results, maps, times, tables = test.test(opt,
